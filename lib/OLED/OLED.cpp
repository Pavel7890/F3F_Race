...
// Added Graphics OLED functions ported from https://www.pvsm.ru/arduino/250799 - 06/01/2020 by V.Urban
// Tested on RS0010
...
void OLED::setGraphicMode()
{
  command(LCD_DISPLAYCONTROL);
  command(LCD_SETGRAPHICMODE);
  command(LCD_CLEARDISPLAY);
  command(LCD_ENTRYMODESET | LCD_DISPLAYCONTROL );
}

void OLED::setCharMode()
{
  command(LCD_DISPLAYCONTROL);
  command(LCD_SETCHARMODE);
  command(LCD_CLEARDISPLAY);
  command(LCD_ENTRYMODESET | LCD_DISPLAYCONTROL);
}
...
// Draws 10x16 dot graphic digits
// digit [0-9]
// pos [0-9] - e.g. only 8 digits per 16 x 2 display (100x16 dots)
void OLED::drawDigit(byte digit, byte pos)
{
  pos *= 10; // 10 dots per position
  for (byte i = pos; i<pos+10; i++)
  {
    setGraphicCursor(i, 0); // First half of digit (top)
    write(GDigits[digit][0][i-pos]);

    setGraphicCursor(i, 1); // Second half of digit (bottom)
    write(GDigits[digit][1][i-pos]);
  }
}

// Draws 3x3 dot graphic
// pos [0-9] - e.g. only 8 digits per 16 x 2 display (100x16 dots)
void OLED::drawDot(byte pos)
{
  pos *= 10;
  for (byte i = pos; i<pos+10; i++)
  {
    setGraphicCursor(i, 0);
    write(GDigit_Dot[0][i-pos]);
    setGraphicCursor(i, 1);
    write(GDigit_Dot[1][i-pos]);
  }
}
...
void OLED::command(uint8_t value) 
{
  send(value, LOW);
  waitForReady();
}
